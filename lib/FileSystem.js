...
class FileSystem {
	...
	static collate(from, to, at, copyGlobs, destGlobs, callback, options) {
		...
		for (var i = 0, l = list.length; i < l; i ++) {
			var srcItem = list[i];
			var shortenedPathSrc = (srcItem.location).substr( (srcItem.location).lastIndexOf(at) + at.length  );
		...
		for (var i = 0, l = list.length; i < l; i ++) {
			var item = list[i];
			var shortenedPath = (item.location).substr( (item.location).lastIndexOf(at) + at.length  );
	...
	static copy(from, to, copyGlobs, callback, options) {
