...
var Glob = module.exports = function(pattern) {
...
Glob.prototype.match = function(Location) {
	var depth = Location.depth;

	var pattern = getPatternAtDepth.call(this, depth);
	...
	if (isMatched) {
		...
		if (this.isNegate) {
			...
			if (this.isDescendAll !== false && this.isDescendAll <= depth) {
				...
				//not at end of pattern depth not matched
				return MATCH_TYPE.NOTMATCHED;	
			}
		}
	}

	function getPatternAtDepth(depth) {
